...
describe "Dynamic Queues" do
  ...
  context "basic queue patterns" do
    ...
    it "can order queues with pattern and traditional wildcard use" do
      Resque.watch_queue("low_z")

      worker = Resque::Worker.new("superhigh_*", "high_*", "*", "low_*")
      worker.queues.should == ["superhigh_z", "high_x", "high_y", "foo", "low_z"]
    end
